...
```{r, dependson="lmFitChunk",fig.height=4,fig.width=4}
lm2 <- lm(movies$score ~ relevel(movies$rating,ref="R")) # XXX relevel
summary(lm2)
```
...
## XXX log(Expected) = linear formula
## Expected value = exp(linear formula) = exp(b0 + b1X) = exp(b0) * exp(b1X)
## => 1 unit increase in X causes expected value to be multiplied by exp(b0)
...
## Rates XXX also works for rates (because rate = count/<total> and taking log introduces an offset term in the linear model)
## log(count) = log(<total>) + b0 + b1X
## the proportion we're modelling below is (hits from simplystatistics on a julian date)/(total hits on that julian date)
...
## What to do
## XXX vcovHC heteroskedasticity consistent
...
##
## bowel cancer example: because there's very small number of people with BC, the values for true
## negative and false positive are really high.  That dilutes the positive predictive values (small
## number of true +ve due to population characteristics/large # of false positives)
...
## XXX matlines below how does it know to plot 95% interval (based on what)?
##
```{r ,dependson="faithlm",fig.height=4,fig.width=4}
...
## XXX type="response" gives the probability directly
...
## Lowess (loess)
## XXX locally weighted scatterplot smoothing
...
## Span
## XXX .1 = 10% of total sample points used, .5 = 50% etc.
##
...
## XXX ??? how to interprete boot.ci results.  e.g where is the confint for the slope term??
